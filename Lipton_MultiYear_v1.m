...
Energy_cons_increase_per_year = 0.03; % linear annual increase in electricity consumption in NL, used with base year 2019 to calculated year. This is due to heat pump, EV, industry electrifying.
...
Load_expected_demand = Load_CBS_2019 .* (1+Energy_cons_increase_per_year*(jaren-2019))
Load_scale = Load_expected_demand / Cons_annual_source_TWh % scale the ETM export consumption to the wanted Load that is programmed in this script which is in line with the Tennet consumption IP2022 estimates.

E_Load_curves  = E_Load_curves .* Load_scale;

Load_jaren = [2019, jaren]
...
if 1 == 2 % histogram van Consumption power NL in 2030 - tussen 12 en 26 GW
    ...
    hold on
    histogram(E_Load_curves(:,end)./1000)
...
P_zon_2022_April = 14800; %[MW] bron Hoogleraar energie Marien
P_zon_2021 = 14349; % [MWp] at end of year 2021. https://solarmagazine.nl/nieuws-zonne-energie/i26353/solar-trendrapport-nederland-installeerde-vorig-jaar-3-6-gigawattpiek-zonnepanelen
P_zon_prognose_2030 = 46200; % [MW] hoog scenario - pv cost down
P_zon_installed_array = [P_zon_2022_April, P_zon_prognose_2030];


% Linear addition PV:
P_zon_annual_addition = (P_zon_prognose_2030 - P_zon_2021) / (2030-2021); % [MWp] additionaly installed PV power in NL (home + large scale buildings + solar fields)
P_zon_installed_array_end_year = ones(1,length(jaren)) .* P_zon_2021 + (jaren-2021).*P_zon_annual_addition; % [MWp] linear addition yearly
P_zon_installed_array_end_year = [P_zon_2021, P_zon_installed_array_end_year];
% end figure 2030: 46.2 GWp
P_zon_installed_array_mid = P_zon_installed_array_end_year(2:end) - diff(P_zon_installed_array_end_year)./2;
P_zon_installed_array = P_zon_installed_array_mid;
...
P_zon_installed_array_end_year = eind_hoogte ./ (1+exp(-helling.*(years - schuiven)))
% end figure: 2030: 46.1 GWp
...
% Go from end year numbers to mid year numbers (as this is the average installed PV or wind power in that year. since installed power is kept constant per year
...
P_wind_2022_April = 5300 + 2460; %[MW] 7.76 GW currently total; onshore + offshore, ratio = 68% wind = onshore (dit zorgt voor kleine fout in modellering, sinds er vanuit ETM bron nu meer offshore dan onshore staat ingesteld)
...
P_wind_prognose_2030 = 8800 + 16700 + (21300-16700)/2; % [MW] medium scenario 8.8GW onshore + 19.0GW offshore

P_onshore_addition = (8800-5300) / (2030-2022);
P_wind_onshore = ones(1,length(jaren)) .* (5300 + P_onshore_addition.*(jaren-2022));


P_wind_offshore = [
2460 + 770  %2022 Hollandse Kust Zuid helft https://nl.wikipedia.org/wiki/Windpark_Hollandse_Kust_Zuid
770  + 759  %2023 Hollandse Kust Zuid helft + Hollandse Kust Noord
0           %2024 
700  + 700  %2025 Hollandse Kust West, kavel VI + Ten noorden van de Waddeneilanden
700         %2026 Hollandse Kust West, kavel VII
0           %2027
1000 + 1000 %2028 IJmuiden Ver, kavel III + IJmuiden Ver, kavel IV 
1000 + 1000 %2029 IJmuiden Ver, kavel I + IJmuiden Ver, kavel II bron: https://tweakers.net/reviews/10042/3/toekomst-van-windenergie-op-zee-megaturbines-als-belangrijkste-energiebron-grote-stappen-vooruit.html
2000 + 2000 %2030 Nederwiek (zuid), kavel I, Nederwiek (noord), kavel II
];

P_wind_offshore_cum = cumsum(P_wind_offshore)';

P_wind_installed_array_end_year = P_wind_onshore + P_wind_offshore_cum;
P_wind_installed_array_mid = P_wind_installed_array_end_year - [750+P_onshore_addition, diff(P_wind_installed_array_end_year)]./2;


if 1 == 2
    figure
    bar(jaren,[P_wind_onshore', P_wind_offshore_cum', P_wind_installed_array_end_year']./1000, 'BarWidth', 2)
    legend('onshore','offshore','total installed wind power NL','Location','northwest')
    ylabel('end-year date installed power GW')
    grid
end


% P_wind_installed_array = [P_wind_2022_April, P_wind_prognose_2030];
P_wind_installed_array = P_wind_installed_array_mid
...
%% Save figure as png
 ...
 save_fig(h0,'Lipton_MultiYear_v1');
