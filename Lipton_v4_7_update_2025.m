...
E_Load_curves = E_Load_curves .* (Cons_CBS_2019/ Cons_annual_source_TWh); % set original back to consumption TWh/y that is true for 2019, so that starting point is correct

Energy_cons_increase_per_year = 0.01;

Load_expected_demand = [Cons_CBS_2019*(1+Energy_cons_increase_per_year*(2022-2019)),... 
                        Cons_CBS_2019*(1+Energy_cons_increase_per_year*(2030-2019))]; %[TWh/year divided by TWh/year] source: https://open-pilot.overheid.nl/repository/ronl-f997136c-6917-4bbd-a2f0-5933f3067f67/1/pdf/bijlage-eindrapport-v2g-waarde-en-weg-voorwaarts.pdf
...
if 1 == 2 % histogram van Consumption power NL in 2030 - tussen 12 en 26 GW
    ...
    save_fig(h_hist_load,'Histogram_consume_2022_2030_v5');
end
...
P_zon_prognose_2030 = 37000; % [MW] medium scenario - pv cost down but with grid congestion and very low bad teruglever tarieven
...
%P_wind_prognose_2030 = 8800 + 16700; % [MW] medium scenario 8.8GW onshore + 16.7GW offshore
P_wind_prognose_2030 = 8800 + 8800; % [MW] laag scenario 8.8GW onshore + 8.8GW offshore
...
gas =       [22;   35]; % €/MWh thermal for 2022 and 2030 scenario
plant_eff = [0.56; 0.49]; % [-] thermal to elec eff
gasplant_fuel_cost = gas./plant_eff;
CO2_price = [85;   180]; % €/tCO2 emitted in 2022 and in 2030
...
fossil_min_price = 30; % €/MWh
gasplant_nom_cost_at_residual_load = 10; % GW
...
price_electricity(residual_load_curves<0) = 0;
%price_electricity(residual_load_curves<0) = exp(-residual_load_curves/1000); % set price to 0 for moments of excess electricity

%price_electricity(residual_load_curves(:,1)<0,1) = exp(-residual_load_curves(:,1)/1000); % set price to 0 for moments of excess electricity
%price_electricity(residual_load_curves(:,2)<0,2) = exp(-residual_load_curves(:,2)/1000); % set price to 0 for moments of excess electricity
...
if 1 == 2 % plot om te controleren:
    ...
    ylabel('Power [GW]')
    grid
    save_fig(h_pv_curt,'PV_ratio_curtailed_v5');
    print -dpng -r300 PV_ratio_curtailed_v5
end
...
if 1 == 2
    ...
    grid
    print -dpng -r300 Zon_Wind_ratio_curtailed_v5
end
...
if 1 == 2  % plot PV price over year 2022
    ...
    print -dpng -r300 PV_price_2022_week_in_April_title_v5
end
...
if 1 == 2  % plot PV price over year 2030
    ...
    grid
    print -dpng -r300 PV_price_2030_week_in_April_v5
end
...
if 1 == 2 % PV dynamic curtailment per year
    ...
    %         ylim([0 250])
    % save_fig(h,'PV curtailment 2030 week in April_v5');
    print -dpng -r300 PV_curtailment_2030_week_in_April_v5
end
...
%% Daily price statistics, 24h grouped histogram, and make difference for winter vs summer, hypothesis: summer daily low prices during day, winter: daily low prices during low load hours at night 00:00-05:00
...
legend('regular 2019 year','2030 whole year','2030 sunny months: 1 march to 1 oct','2030 darker winter months')
 ...
 save_fig(h,'2030 daily hourly avg electricity price_v5');
end 
...
if 1 == 2
    ...
    jaar = 2;
    title(sprintf('Demand: %.1f TWh, %.1f GW Wind, %.1f GWp PV, Wind gen: %.0fprct, PV gen: %.0fprct', Cons_annual(jaar)/1000, P_wind_installed_array(jaar)/1000  ,P_zon_installed_array(jaar)/1000,  Prod_wind_perc(jaar)*100,  Prod_solar_perc(jaar)*100) )
    save_fig(h2030,'Zon_scatterplot_2030_v5');
    print -dpng -r300 Zon_scatterplot_2030_v5
end
...
%% initiliaze plot
h0 = figure('Name','Electricity market NL 2030','pos',[0 0 1800 1200]); % width and height start and end points
tiledlayout(4,2)
...
for jaar = 1:2
    ...
    share_participate_V2G = 0.12; %[-]
    n_vehicles_V2G = n_vehicles * share_participate_V2G; % number of vehicles participating in V2G
    share_connected_to_charge_pole = 0.75; % connected to charge pole on avg
    ...
    BESS_max_charge_P = n_vehicles_V2G_connected * OBC_power % [MW]

    E_vehicle = 60e-3; %[MWh] storage per vehicle = 65kWh
    E_vehicle_V2G_part = 0.50; %[-] 50% of SoC is set to be available for V2G
    BESS_energy_storage = n_vehicles_V2G * E_vehicle * E_vehicle_V2G_part % [MWh] all V2G vehicles determine max total energy charged - but power is limited by V2G share AND charge pole share

    BESS_duration = BESS_energy_storage ./ BESS_max_charge_P     %[hours]
    ...
    for a = 1:(length(time_array)-1)
        ...
        if residual_load_curve(a) < 0 % thus excess energy, than storage is charged
            ...
            if residual_load_curve(a) < -BESS_max_charge_P % check charge power limit
                Storage_V2G(a+1) = Storage_V2G(a) + BESS_max_charge_P; % limit storage charging to max power and add energy stored
                P_V2G_charge(a+1) = BESS_max_charge_P;
            end
            if Storage_V2G(a) > BESS_energy_storage % limit Storage capacity (270GWh is 0.9M EV's with 290kWh V2G volume/year)
                Storage_V2G(a) = BESS_energy_storage;
                Storage_V2G(a+1) = BESS_energy_storage;
                P_V2G_charge(a) = 0;
        ...
        elseif residual_load_curve(a) > 0 % thus shortage of energy - fossil back up required
            ...
            if Storage_V2G(a) > 0 % make sure storage can not deplete more than was charged before
                ...
                if residual_load_curve(a) > BESS_max_charge_P
                    Storage_V2G(a+1) = Storage_V2G(a) - BESS_max_charge_P;
                    P_V2G_discharge(a) = BESS_max_charge_P;
                end
            else
...
rx1 = nexttile;
    ...
    legend('Storage discharge','Residual load (mainly fossil backup)','Storage charge','PV solar (residential, commercial, central)','Wind energy (inland, coastal, offshore)','Consumption','Consumption (incl flexible)')
    grid
    ...
    grid
    ylim([0 50])
    legend('Unlimited storage charging on excess energy',sprintf('Storage: %.1f GWh, with power: %.1f GW AC',BESS_energy_storage/1000, BESS_max_charge_P/1000))
...
% Save figure as pdf
% save_fig(h0,'Lipton_PDF_v5');     % uses minimized edge borders

%% Save figure as png

 save_fig(h0,'Lipton_v5');

% print -dpng -r300 Lipton_v5
...
%%
%save_fig(h8,'Matlab-Day-Ahead-prices-2022-2030_v5');
...
%writetable(price_electricity_table,'Matlab-Day-Ahead-prices-2022-2030_table_v5.xlsx')
%writetable(price_electricity_table,'Matlab-Day-Ahead-prices-2022-2030_table_v5.csv')
